...
const DormantEmployeesPage: React.FC = () => {
    ...
    const fetchDormantEmployees = async () => {
        if (companyId) {
            try {
                const response = await axios.get(`${BACKEND_URL}/api/${companyId}/dormant-employees`);
    ...
    const reactivateEmployee = async (applicantId: number) => {
        try {
            await axios.patch(`${BACKEND_URL}/api/employees/${applicantId}/dormant`, { dormant: false });
